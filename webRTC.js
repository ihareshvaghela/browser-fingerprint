...
function rejectPromises() {
    promises.forEach(({ reject }) =>
        reject('This browser is not supported, so WebRTC2 cannot provide a unique, static ID for this machine.')
    )
    promises.length = 0
}
...
export async function WebRTCUniqueId() {
    if (typeof RTCPeerConnection == 'undefined')
        throw new Error(`This browser doesn't support WebRTC, so webRTC cannot provide a unique, static ID for this machine.`)
